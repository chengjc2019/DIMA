...
class ProRotatedConvFCBBoxHead(RotatedBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(ProRotatedConvFCBBoxHead, self).__init__(
        ...
        if self.with_cls:
            ...
            self.fc_cls_fine = build_linear_layer(
                self.cls_predictor_cfg,
                in_features=6,
                out_features=38)
        if self.with_reg:
    ...
    def forward(self, x, coarse_cls_score):
        """Forward function."""
        ...
        fine_cls_score = self.fc_cls_fine(coarse_cls_score)
        # fine_cls_score = self.fc_cls(coarse_cls_score)
        bbox_pred = self.fc_reg(x_reg) if self.with_reg else None
        # return cls_score, bbox_pred
        # cls_score_ = cls_score*fine_cls_score
        # return cls_score_
        return fine_cls_score


@ROTATED_HEADS.register_module()
class ProRotatedShared2FCBBoxHead(ProRotatedConvFCBBoxHead):
    """Shared2FC RBBox head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(ProRotatedShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
