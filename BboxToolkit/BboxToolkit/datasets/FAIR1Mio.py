    ...
    if txtfile is None:
        ...
        with open(txtfile, 'r') as f:
            for line in f:
                ...
                #         # print(True)
                #         continue
                #     bboxes.append([float(i) for i in items[0:8]])
                #     labels.append(cls2lbl[items[8]])
                #     diffs.append(int(items[9]) if len(items) == 10 else 0)
                if len(items) >= 10:
                    if items[9] not in cls2lbl:
                        continue
                    bboxes.append([float(i) for i in items[1:9]])
                    labels.append(cls2lbl[items[9]])
                    diffs.append(int(items[10]) if len(items) == 11 else 0)
...
def save_fair1m_submission(save_dir, id_list, dets_list, task='Task1', classes=None, with_zipfile=True):
    ...
    files = [osp.join(save_dir, cls + '.txt') for cls in classes]
