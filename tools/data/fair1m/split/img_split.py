...
import BboxToolkit.datasets.io as saveio
import BboxToolkit.datasets.DOTAio as dotaio
import BboxToolkit.datasets.misc as mi
        ...
        with codecs.open(outdir, 'w', 'utf-8') as f_out:
            if bboxes_num == 0:
                ...
                for idx in range(bboxes_num):
                    ...
                    outline = outline + ' ' + obj['labels'][idx].strip() + ' ' + diffs
                    f_out.write(outline + '\n')
...
def load_dota(img_dir, ann_dir=None, nproc=10):
    ...
    if nproc > 1:
        ...
        # contents = mi.prog_map(_load_func, imgs, nproc)
        pool.close()
    else:
...
def main():
    ...
    CLASSES = ('Boeing737', 'Boeing747', 'Boeing777', 'Boeing787', 'C919', 'A220', 'A321', 'A330', 'A350', 'ARJ21',
               ...
               'Dump-Truck', 'Van', 'Trailer', 'Tractor', 'Excavator', 'Truck-Tractor', 'other-vehicle',
               'Basketball-Court',
    ...
    # img_dirs = list(os.listdir(args.img_dirs[0]))
    infos, img_dirs = [], []
    for img_dir, ann_dir in zip(args.img_dirs, args.ann_dirs):
        # img_dir = args.img_dirs+img_dir
        _infos = load_dota(img_dir=img_dir, ann_dir=ann_dir, nproc=args.nproc)
        # _infos = dotaio.load_dota(img_dir=img_dir, ann_dir=ann_dir, classes=classes, nproc=args.nproc)
    ...
    saveio.save_pkl(osp.join(save_files, 'ori_annfile.pkl'), infos, classes)
    saveio.save_pkl(osp.join(save_files, 'patch_annfile.pkl'), patch_infos, classes)
